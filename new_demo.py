...
class Font(object):
    ...
    def calculate_width_interactive(self, image_path, img, skeleton):
        ...
        white_mask=np.full((width, height,3), 255, dtype=np.uint8)
        contours, hierarchy = cv2.findContours(img_bin, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(white_mask, contours, -1, (0, 0, 0), 2)
        ...
        def mouse_callback(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                if closest_skeleton_point is not None:
                    cv2.circle(white_mask, closest_skeleton_point, 2, (0, 255, 0), -1)
                    # 计算最近点的内切圆
                    radius = self.calculate_inner_circle(closest_skeleton_point, img_bin, width, height)
                    
                    # 在原图上画出内切圆
                    cv2.circle(white_mask, closest_skeleton_point, radius, (0, 0, 255), 1)
                    
                    # 显示圆心坐标
                    # text_position = (center[0] + 5, center[1] - 5)
                    centerx, centery = closest_skeleton_point
                    cv2.putText(white_mask, f"({centerx}, {centery})", (closest_skeleton_point[0]-20,closest_skeleton_point[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.2, (0, 0, 255), 1, cv2.LINE_AA)

                
                cv2.imshow("Character Width", white_mask)
